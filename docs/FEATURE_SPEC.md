...
### F-0.8 온보딩 흐름 (신규 사용자 시작 안내)
- **환영 모달**: 완전 신규 사용자(콘텐츠·학습·복습 0) 첫 로그인 시 1회 표시(닫힘 `localStorage`). 3단계 소개 + "첫 콘텐츠 추가" CTA
- **시작 체크리스트**: 대시보드 상단 카드. ① 콘텐츠 추가 ② 첫 학습 ③ 첫 복습 — `GET /api/onboarding`이 실제 데이터(`videos`/`study_log`)로 완료 여부 판정. 3단계 완료 시 자동 소멸, 수동 닫기(✕) 제공
- **원칙**: 온보딩 상태를 별도 저장하지 않고 실제 활동으로 계산 → 마이그레이션 불필요·항상 정확
...
- **보안**: 세션 쿠키 `HttpOnly`+`SameSite=Lax`(+프로덕션 `Secure`), 로그인 brute-force 레이트리밋(IP 기준 15분 8회 초과 429)

### F-0.1.1 비밀번호 재설정
- **엔드포인트**: `POST /api/auth/forgot-password`(재설정 메일 요청), `GET /reset-password`(페이지), `POST /api/auth/reset-password`
- **토큰**: `users.reset_token`/`reset_sent_at`, **1시간 만료 + 1회용**(성공 시 소거)
- **보안**: 이메일 열거 방지(가입 여부 무관 동일 응답), OAuth 전용 계정 제외, 재설정 성공 시 `email_verified=1`+자동 로그인
- **UI**: `login.html` "비밀번호를 잊으셨나요?" → 재설정 메일 요청 / `templates/reset.html` 새 비밀번호 입력
...
| v1.6 | 2026-07-07 | 온보딩 흐름 — 신규 사용자 환영 모달 + 대시보드 시작 체크리스트(3단계 자동 추적) |
| v1.7 | 2026-07-07 | 보안 강화 — 세션 쿠키 플래그(HttpOnly/SameSite/Secure) + 로그인 brute-force 방지(429) |
| v1.8 | 2026-07-07 | 비밀번호 재설정 — 재설정 메일(1시간 만료 토큰) → 새 비밀번호, 이메일 열거 방지 |
